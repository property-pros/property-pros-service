...
pre-test-api:
	make kill-app && \
	while ! curl --output /dev/null --silent --head --fail http://localhost:8083; do sleep .2; done && \
	echo "listening on port 8083"
test-api:
	make pre-test-api && \
	echo "starting tests..." && \
	export PROJECT_PATH=$(shell pwd) TEST_SERVICE="localhost:8083" && \
...
release:
	./cicd/release.sh
cleardb:
	@psql "postgres://qtqttpfd:Z2QX9jAADkYsGiXFWKydrKpGc3gOTBpN@bubble.db.elephantsql.com/qtqttpfd" -c "DELETE FROM note_purchase_agreements WHERE id = 'test-npa-1';"